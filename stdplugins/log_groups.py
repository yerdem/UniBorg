...
NO_PM_LOG_USERS = []
PM_LOGGR_BOT_API_ID = -1001306489849

@borg.on(events.NewMessage(incoming=True, func=lambda e: e.is_group and  e.media))
async def monito_p_m_s(event):
    ...
    if Config.NC_LOG_P_M_S and not sender.bot:
        ...
        if chat.id not in NO_PM_LOG_USERS and chat.id != borg.uid:
            try:
                e = await borg.get_entity(PM_LOGGR_BOT_API_ID)
